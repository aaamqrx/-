...
def build_baidu_features(cache_data):
    ...
    mean_value = sum(numeric_values) / n if n else pd.NA

    # 衍生特征：标准差、趋势斜率、最大值、最小值
    if n >= 2:
        std_value = float(np.std(numeric_values, ddof=0))
        # 线性趋势斜率：用最小二乘法
        x_days = np.arange(n, dtype=float)
        x_mean = x_days.mean()
        y_mean = mean_value
        numerator = np.sum((x_days - x_mean) * (np.array(numeric_values) - y_mean))
        denominator = np.sum((x_days - x_mean) ** 2)
        trend_slope = float(numerator / denominator) if denominator > 0 else 0.0
    else:
        std_value = 0.0
        trend_slope = 0.0

    max_value = max(numeric_values) if n else pd.NA
    min_value = min(numeric_values) if n else pd.NA
